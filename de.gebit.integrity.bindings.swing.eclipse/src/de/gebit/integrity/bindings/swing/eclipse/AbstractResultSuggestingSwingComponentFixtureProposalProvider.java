...
/**
 * Abstract base class for proposal providers which propose possible results
 * based on data currently found in the application. This one is geared towards
 * a specific "list"-style pattern.
 * 
 * @author Slartibartfast
 * 
 */
public abstract class AbstractResultSuggestingSwingComponentFixtureProposalProvider
		extends AbstractSwingComponentFixtureProposalProvider {
	...
	/**
	 * Returns the author assist request type to send to the author assist
	 * server.
	 * 
	 * @return
	 */
	...
	public List<CustomProposalDefinition> defineResultProposals(
		...
		if (Arrays.asList(getRelevantMethods()).contains(aFixtureMethodName)) {
			...
			return super.defineResultProposals(aFixtureMethodName, aResultName,
					aResultValue, someParameterValues);
		}
	}

	private List<CustomProposalDefinition> requestEntryProposals(
			Map<String, Object> someParameterValues) {
		String tempComponentName = (String) someParameterValues
				.get(AbstractSwingFixture.COMPONENT_PATH_PARAMETER_NAME);
